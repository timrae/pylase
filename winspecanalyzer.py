...
try:
    from winspec import Winspec, CommError
...
WINSPEC_MAX_MEAS_TIME=20.0
# List of tuples containing the filter wheel position, gain, and exposure time
GAIN_SETTINGS=[(1, 2, WINSPEC_MAX_MEAS_TIME), (1, 2, 10.0), (1, 2, 1.0), (1, 2, 0.1), (1, 1, 0.2), (1, 1, 0.02), (2, 1, 0.2), (3, 1, 0.2), (4, 1, 0.2), (5, 1, 0.2), (5, 1, 2e-2), (5, 1, 2e-3), (5, 1, 2e-4)]
...
DEFAULT_PATH=r"C:\Winspec"      # Default path to put Winspec files
...
def getNumberOfPixels():
    """ Pass the number of pixels in the detector in winspec """
    # I'd like a way to store this in the winspec module without getting errors from not having the correct libraries installed
    return 1024

def getPixelResolutions():
    """ Pass the average resolution of each pixel in the detector in winspec for each grating """
    return [x/1000 for x in [20.8,71.1,323.8]] # in nm
...
class WinspecAnalyzer(QtCore.QObject):
    ...
    def __init__(self,inputStateSwitch=None,rawDataDir=DEFAULT_PATH,*args,**kwargs):
    ...
    def readPowerAuto(self,*args, **kwargs):
        ...
        wavelength,counts, dict=self.readWinspecSpectrumAuto(*args,**kwargs)
    ...
    def obtainSpectrum(self,tau=DEFAULT_TAU,calibratedPower=None,timeout=DEFAULT_TIMEOUT):
        """ Acquire a spectrum using center,span,and resolution set in the instance object. 
        ...
        if self.numSpectra == 1:
            wavelength,counts,spectrumDict=self.readWinspecSpectrumAuto(tau)
        else:
            # Setup empty variables to hold the data
            pixels=getNumberOfPixels()
            ...
            wavelength_i,counts_i,spectrumDict=self.readWinspecSpectrumAuto(tau)
            ...
            for idx in range(self.numSpectra):
                if idx != centerIndex:
                    ...
                    wavelength_i,counts_i,spectrumDict_i=self.readWinspecSpectrumAuto(tau,rangeMode="fixed")
    ...
    def readWinspecSpectrumAuto(self,tau=DEFAULT_TAU,timeout=DEFAULT_TIMEOUT,rangeMode="auto", mode=None):
        ...
        while 1:
            try:
                wavelength,counts,spectrumDict=self._readSpectrumAuto(tau,timeout,rangeMode)
                break
    ...
    def _readSpectrumAuto(self,tau,timeout,rangeMode):
        ...
        wavelengthData,counts,spectrumDict=self.acquireData(self.accumulations(tau))    
        # Increase the range if saturating
        if spectrumDict["saturating"]:
            if self.rangeIndex<(len(GAIN_SETTINGS)-1) and rangeMode!="fixed":
                self.setRange(self.rangeIndex+1)
                return self._readSpectrumAuto(tau,timeout,rangeMode)
            else:
                raise CommError, "The measured power was outside the measurement range with rangeMode="+rangeMode
    ...
    def acquireData(self,*args,**kwargs):
        """ Wrapper method to Winspec.acquireData() in winspec module including automatic background subtraction """
        if self.bgMeasRequired:
            # Turn off the input signal, measure background, then turn it back on again
            self.inputStateSwitch(False)
            self._connection.acquireBackground()
            self.inputStateSwitch(True) 
            self.bgMeasRequired = False
        return self._connection.acquireSpectrum(*args,**kwargs)
    ...
    def setNumPoints(self,numPoints):
        ...
        self.numSpectra=int(numPoints/getNumberOfPixels())
        
    def setResolution(self,resolution):
        """ Set the resolution in nm (i.e. grating) to use for measurements """
        self._setGratingNumber(int((abs(np.array(getPixelResolutions())-resolution)<1e-3).nonzero()[0][0])+1)
    ...
    def accumulations(self,tau):
        ...
        return np.ceil(tau/1000/GAIN_SETTINGS[self.rangeIndex][2])
